/**
 * Manual script to populate news database
 * Calls Exa API and the deployed dashboard service
 */
...
async function manualNewsSync() {
  console.log('📰 Manual News Sync: Starting direct database population\n');
  ...
  for (const mapping of policyInterests) {
    ...
    try {
      ...
      // Store articles
      if (exaData.results) {
        for (const article of exaData.results) {
          const url = new URL(article.url);
          const sourceDomain = url.hostname.replace('www.', '');

          console.log(`   - ${article.title?.substring(0, 60)}... (${sourceDomain})`);
        }
      }
  ...
  console.log('\n⚠️  Note: Articles were fetched but need database access to store them.');
  console.log('The dashboard service endpoint /admin/sync-news would handle storage.');
}
