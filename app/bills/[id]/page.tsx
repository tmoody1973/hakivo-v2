...
export default function BillDetailPage() {
  ...
  useEffect(() => {
    async function fetchBill() {
      try {
        ...
        console.log('[BillDetailPage] NEXT_PUBLIC_DASHBOARD_API_URL:', process.env.NEXT_PUBLIC_DASHBOARD_API_URL)
        ...
        if (response.success && response.data) {
          ...
          // Don't show error for auth issues - useEffect will retry when token is ready
          if (response.error?.code === 'UNAUTHORIZED') {
            // Keep loading state - will retry when accessToken changes
            console.log('[BillDetailPage] Auth not ready, staying in loading state')
          } else {
  ...
  useEffect(() => {
    // Only poll if we have a bill and auth token
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-3">
                        <div>
                          ...
                          <p className="text-lg font-bold">{analysis.fiscalImpact.estimatedCost}</p>
                        </div>
                        <div>
                          <p className="text-xs font-semibold text-muted-foreground uppercase">Funding Source</p>
                          <p className="text-sm text-muted-foreground">{analysis.fiscalImpact.fundingSource}</p>
                        </div>
                        <div>
                          <p className="text-xs font-semibold text-muted-foreground uppercase">Timeframe</p>
                          <p className="text-sm text-muted-foreground">{analysis.fiscalImpact.timeframe}</p>
                        </div>
                      </CardContent>
                    </Card>
                  )}
                        ...
                        <div className="space-y-3">
                          {Object.entries(analysis.stakeholderImpact).map(([stakeholder, impact]) => (
                            <div key={stakeholder} className="border-l-2 border-primary pl-4">
                              <h4 className="font-semibold text-sm capitalize">{stakeholder}</h4>
